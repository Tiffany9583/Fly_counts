...
def update_tracks(tracker, frame_count, save_txt, txt_path, save_img, view_img, im0, gn, fly_counts):
    ...
    for track in tracker.tracks:
        ...
        class_name = names[int(
            class_num)] if opt.detection_engine == "yolov5" or "yolov7" else class_num
...
def detect(save_img=False):
    ...
    encoder = gdet.create_box_encoder(
        model, transform, batch_size=1, device=device)
    ...
    if opt.detection_engine == "yolov5":
        yolov5_engine = Yolov5Engine(opt.weights, device, opt.classes,
                                     opt.confidence, opt.overlap, opt.agnostic_nms, opt.augment, half)
        global names
        names = yolov5_engine.get_names()
    # load yolov7 model here
    elif opt.detection_engine == "yolov7":
        yolov7_engine = Yolov7Engine(opt.weights, device, opt.classes,
                                     opt.confidence, opt.overlap, opt.agnostic_nms, opt.augment, half)
        names = yolov7_engine.get_names()
    elif opt.detection_engine == "yolov4":
        yolov4_engine = Yolov4Engine(opt.weights, opt.cfg, device, opt.names, opt.classes,
                                     opt.confidence, opt.overlap, opt.agnostic_nms, opt.augment, half)
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        if opt.detection_engine == "roboflow":
            pred, classes = predict_image(
                im0, opt.api_key, opt.url, opt.confidence, opt.overlap, frame_count)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--detection-engine", default="roboflow",
                        help="Which engine you want to use for object detection (yolov7, yolov5, yolov4, roboflow).")
